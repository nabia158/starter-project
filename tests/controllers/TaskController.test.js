...
describe("TaskController", () => {
  ...
  describe("Task Retrieval", () => {
    ...
    test("should filter tasks by status", () => {
      ...
      taskController.updateTask(completedResponse.data.id, {
        status: "completed",
      });

      // Act
      const pendingResponse = taskController.getTasks({ status: "pending" });
      const completedResponse2 = taskController.getTasks({
        status: "completed",
      });
  ...
  describe("Task Updates", () => {
    ...
    test("should fail to update non-existent task", () => {
      // Act
      const response = taskController.updateTask("non-existent-id", {
        title: "Test",
      });
  ...
  describe("Permission Testing", () => {
    ...
    test("should not allow updating other user task", () => {
      // Act
      const response = taskController.updateTask(otherUserTask.id, {
        title: "Hacked",
      });
